...
// This structure the driver will receive after loading
// and by its means will get access to some abstract nIRQ GPIO value
// Frankly speaking, power control should be here as well with some
// function pointers passed in driver, but I`m too lazy for that,
// and that`s why the driver will know PWRD GPIO number
static const struct i2c_board_info jdts_temperature_sensor_board_info[] = {
	{
		I2C_BOARD_INFO("jdts",0x55),
		.irq = TEGRA_GPIO_TO_IRQ(TEGRA_GPIO_PBB0)
	},
};

// here we tune up our GPIO (yes, again, to be sure)
// Since I do not know how and is implemented in the propiretary code,
// I set pins states with Linux way via standard gpio interface
static int grouper_jdts_init(void)
{
	// Nevertheless, here we again have to address some magic of nVidia 
   	// tegra_gpio_enable, otherwise the pins won`t enable

	int ret = 0;

	ret = gpio_request(TEGRA_GPIO_PBB5, "jdts_npwd");
	if (ret < 0) {
	  pr_err("TechartMicroSystems JDTS: Error: cannot register GPIO_PWR_DOWN\n");
	}
	else {
		ret = gpio_direction_output(TEGRA_GPIO_PBB5, true);
		if (ret < 0) {
		  pr_err("TechartMicroSystems JDTS: Error: cannot set GPIO_PWR_DOWN as output\n");
		} 
		else {
	...
	ret = gpio_request(TEGRA_GPIO_PBB0, "jdts_nirq");
	if (ret < 0) {
		pr_err("TechartMicroSystems JDTS: Error: cannot register GPIO_NIRQ\n");
		return ret;
	}
	...
	if (ret < 0) {
		...
		pr_err("TechartMicroSystems JDTS: Error: cannot set GPIO_NIRQ as input\n");
	}
	else {
		tegra_gpio_enable(TEGRA_GPIO_PBB0);
	}

	printk("%s: TechartMicroSystems JDTS OK", __FUNCTION__);
...
// Finally, here is a overall init function to start the sensors on the board
// We call here our pins configuration and register the sensor
...
/* Front Camera mi1040 + */
    ...
    i2c_register_board_info(2/*sensor I2C bus number*/, jdts_temperature_sensor_board_info,
    	ARRAY_SIZE(jdts_temperature_sensor_board_info));
