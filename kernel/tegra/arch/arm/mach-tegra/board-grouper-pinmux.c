...
static __initdata struct tegra_pingroup_config grouper_pinmux_common[] = {
	...
	// Found previously pins in the schematics says that target pins had names
	// GPIO_PBB0 and GPIO_PBB5. They are here, in camera section, which is absent in our device
	...
	// after: interruption pin is in pulled up, since it is nIRQ
	...
	// after: power control pin is out pulled down, as to keep it off by default
...
// A small pin initialization table
// Even though the comment below tells us not to waste our time
// the code after it does not look not working, so let`s add some 
// initialization changes there too
...
static struct gpio_init_pin_info init_gpio_mode_grouper_common[] = {
   ...
   // our pins are here. See further to find why so :)
...
// This function is called from the one after and applies the smaller pins table
static void __init grouper_gpio_init_configure(void)
{
   ...
   if (project_info == GROUPER_PROJECT_NAKASI_3G) {
      ...
      // here we are - our model is not 3G, our tablet has got no 3G onboard
...
// One of kernel init functions. 
// Here all pinmux table will go into proprietary nVidia part
